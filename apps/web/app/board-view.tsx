    ...
    <>
      {!hasRows ? (
        <div className="empty">
          ...
          <ol>
            ...
            <li>
              In Settings, choose <strong>Add a device</strong> and run the displayed command.
            </li>
            <li>
              <code>npm run usurp -- sync</code>
            </li>
          </ol>
        </div>
      ) : (
        <div className="table-scroll" role="region" aria-label="Burn standings" tabIndex={0}><table className="board">
          <thead>
            <tr>
              <th scope="col">#</th>
              <th scope="col">Who</th>
          ...
          <tbody>
            {board.rows.map((row) => (
              <tr key={`${row.rank}-${row.handle ?? row.pseudonym}`}>
                ...
                <td className="num" title={row.effectiveTokens.toLocaleString()}>
                  {compact(row.effectiveTokens)}
                </td>
                <td className="num" title={row.outputTokens.toLocaleString()}>
                  {compact(row.outputTokens)}
                </td>
                <td className="num sub" title={row.cacheReadTokens.toLocaleString()}>
                ...
                <td>
                  <span className={row.flagged ? "badge flagged" : "badge"}>
                    {row.flagged ? "flagged" : row.trustTier.replace("_", " ")}
                  </span>
                </td>
              </tr>
            ))}
      ...
      <footer>
        <p>
          ...
          Cache&nbsp;reads are shown separately because they are ~10% of the input
          rate — counting them as volume is what rewards waste.
        </p>
        <p>
