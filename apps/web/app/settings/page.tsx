...
export default async function SettingsPage({
  ...
  const revealId = typeof params.reveal === "string" ? params.reveal : undefined;
  const revealed = revealId ? await revealChannelSecret(db, user.id, revealId) : null;
  ...
  return (
    <main className="wrap settings-page">
      <header className="masthead">
        ...
        <div className="row">
          <a className="tab" href={`/u/${encodeURIComponent(user.handle)}?window=all`}>
            View my usage ↗
          </a>
      ...
      {banner && (
        <p className={banner.ok ? "disclaimer" : "disclaimer error"}>{banner.message}</p>
      )}

      <div className="settings-layout">
      <nav className="settings-nav" aria-label="Settings sections">
        <p className="eyebrow">Preferences</p>
        <a href="#profile">Profile</a><a href="#devices">Devices & sync</a><a href="#arenas">Arenas & visibility</a><a href="#clubs">Clubs</a><a href="#notifications">Notifications</a><a href="#security">Account security</a>
      </nav>
      ...
      {!user.handleConfirmed && (
        <p className="field-hint">
          This handle was generated from your provider profile. Saving it below
          confirms it as yours.
        </p>
      )}
      <form className="form" action={updateHandleAction}>
        <label className="field">
          ...
          <span className="field-hint">
            Your profile lives at <code>/u/{user.handle}</code>. Letters, digits,{" "}
      ...
      {deviceRows.length === 0 ? (
        ...
        <div className="table-scroll" role="region" aria-label="Connected devices" tabIndex={0}><table className="board">
          <thead>
            <tr>
          ...
          <tbody>
            {deviceRows.map((d) => (
              <tr key={d.id}>
                ...
                <td>
                  <span className={d.revokedAt ? "badge flagged" : "badge"}>
      ...
      <p className="field-hint">Choose where you compete and how you appear. Each arena has its own visibility setting.</p>

      {!inGlobal && (
        <div className="empty">
          <h2>You are not competing globally</h2>
          <p>
            Global membership is opt-in. Joining names you on the public board
            unless you pick anonymous or hidden afterwards.
          </p>
            ...
            <div className="card" key={m.arena.id}>
              <div className="card-head">
                <div>
                  ...
                  <span className="sub">
                    {m.memberCount}
                    {m.arena.maxMembers ? ` / ${m.arena.maxMembers}` : ""} members
                  </span>
                </div>
              </div>
              ...
              {m.arena.type === "org" && (
                <p className="field-hint">
                  Org arenas default to <strong>hidden</strong>. Admins see
                  aggregates only unless you set yourself public here.
                </p>
              )}

              <form className="row" action={setVisibilityAction}>
                <input type="hidden" name="arena_id" value={m.arena.id} />
                <select className="input select" name="visibility" defaultValue={m.visibility} aria-label={`Visibility in ${m.arena.name}`}>
              ...
              <p className="field-hint">{VISIBILITY_HELP[m.visibility]}</p>
        ...
        <form className="form" action={createClubAction}>
          <label className="field">
            ...
            <input
              ...
              maxLength={48}
            />
            <span className="field-hint">Up to 50 members. You get an invite code.</span>
          </label>
          <button className="button" type="submit">
            Create
          </button>
        ...
        <form className="form" action={joinClubAction}>
          <label className="field">
            ...
            <input
              ...
              spellCheck={false}
            />
            <span className="field-hint">Spacing, dashes and case are ignored.</span>
          </label>
          <button className="button secondary" type="submit">
            Join
          </button>
      ...
      <p className="field-hint">
        Get updates when you take or lose the Throne, or a duel changes. At most one throne alert per
        arena per{" "}
        {THRONE_COOLDOWN_MS === 3_600_000 ? "hour" : `${THRONE_COOLDOWN_MS / 60000} minutes`}.
      </p>

      {channels.length > 0 && (
        <div className="table-scroll" role="region" aria-label="Notification channels" tabIndex={0}><table className="board">
          <thead>
            <tr>
          ...
          <tbody>
            {channels.map((c) => (
              <tr key={c.id}>
                ...
                <td>
                  <span className={c.enabled ? "badge" : "badge flagged"}>
                    {c.enabled ? (c.signed ? "signed" : "enabled") : "paused"}
                  </span>
                </td>
      ...
      <form action={addChannelAction} className="row" style={{ marginTop: 14 }}>
        <select className="input" name="kind" defaultValue="webhook" aria-label="Channel type">
      ...
      <p className="field-hint">
        Webhooks use a signing secret shown once when you add the channel. Email delivery is not available on this deployment.
      </p>
      </section>
